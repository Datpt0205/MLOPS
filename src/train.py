...
def train_and_log():
    ...
    print(
        f"MLflow Tracking URI (mac dinh): {mlflow.get_tracking_uri()} -> ./mlruns"
    )  # Sẽ dùng local ./mlruns
    ...
    if not MODEL_PARAMS:
        print(
            f"CANH BAO: Khong tim thay cau hinh sieu tham so cho model type '{MODEL_TYPE}'. Chay 1 lan voi tham so mac dinh."
        )
        param_combinations = [{}]
    else:
        ...
        try:
            with mlflow.start_run() as run:
                ...
                if MODEL_TYPE == "LogisticRegression":
                    model_instance = LogisticRegression(
                        **params, random_state=42, max_iter=1500
                    )
            ...
            if mlflow.active_run():
                try:
                    ...
                    print(f"Loi khi ket thuc run {run_id}: {e_log}")
