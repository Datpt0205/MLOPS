...
def predict():
    ...
    try:
        ...
        if not data or 'features' not in data or not isinstance(data['features'], list) or len(data['features']) != N_FEATURES:
            return jsonify({"error": f"Du lieu khong hop le. Can key 'features' la list co {N_FEATURES} phan tu so."}), 400

        # Validate kiểu dữ liệu features
        features = data['features']
        if not all(isinstance(x, (int, float)) for x in features):
             return jsonify({"error": "Tat ca cac phan tu trong 'features' phai la so (int hoac float)."}), 400
        ...
        return jsonify({"predictions": predictions_list, "model_served_from_path": MODEL_PATH})
    except Exception as e:
...
def predict_batch():
    ...
    try:
        ...
        if not data or 'instances' not in data or not isinstance(data['instances'], list):
            return jsonify({"error": "Du lieu khong hop le. Can key 'instances' la mot list."}), 400

        all_features = []
        for i, instance in enumerate(data['instances']):
             if not isinstance(instance, dict) or 'features' not in instance or not isinstance(instance['features'], list) or len(instance['features']) != N_FEATURES:
                  return jsonify({"error": f"Instance {i} khong hop le. Can key 'features' la list co {N_FEATURES} phan tu so."}), 400
             if not all(isinstance(x, (int, float)) for x in instance['features']):
                  return jsonify({"error": f"Instance {i}: Tat ca features phai la so."}), 400
             all_features.append(instance['features'])

        if not all_features:
             return jsonify({"predictions": [], "model_served_from_path": MODEL_PATH}) # Trả về rỗng nếu input rỗng

        input_df = pd.DataFrame(all_features)
        predictions = model.predict(input_df)
        predictions_list = predictions.tolist() if isinstance(predictions, np.ndarray) else predictions
        print(f"Input batch ({len(all_features)} samples), Predictions: {predictions_list}")
        return jsonify({"predictions": predictions_list, "model_served_from_path": MODEL_PATH})
